...
# Ideal angles for shoulder press
ideal_top_angle = 180
...
def generate_frames():
    ...
    if username:
        ...
        count_goal = fitness_levels.get(fitness_level, 8)  # Default goal if fitness level is unrecognized
    else:
        count_goal = 8  # Default count goal
    ...
    with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
        while cap.isOpened():
            ...
            try:
                ...
                # Coordinates for shoulders, elbows, wrists
                ...
                # Shoulder press counting logic
                if arm_angle > 160:  # Top position
                    stage = "up"
                if arm_angle < 90 and stage == "up":  # Bottom position
                    stage = "down"
                    counter += 1

                # Display information
                cv2.rectangle(image, (0, 0), (500, 250), (245, 117, 16), -1)
                cv2.putText(image, f'Shoulder Press Count: {counter}', (15, 50), cv2.FONT_HERSHEY_SIMPLEX, 1.2, (255, 255, 255), 2)
